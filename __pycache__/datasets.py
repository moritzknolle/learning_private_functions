...
def load_citibike_data(
    ...
    Args:
        ...
        mechanism (str): privacy mechanism to use, one of "lap", "gauss" or "gauss_analytic"
    ...
    if mechanism == "lap":
        mech = diffprivlib.mechanisms.LaPlace(epsilon=eps, delta=delta, sensitivity=se)
    elif mechanism == "gauss":
        mech = gaussian_mechanism
    elif mechanism == "gauss_analytic":
        raise NotImplementedError()

    # clip then privatise targets
    if private_targets:
        y_priv = mech(
            inp_arr=np.clip(y, a_min=0, a_max=2000),
            eps=eps,
            delta=delta,
            sens=MAX_TRIP_DURATION,
        )
        y = y_priv
        # y_diff = y_clipped - y
