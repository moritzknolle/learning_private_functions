# adapted from https://github.com/tensorflow/privacy/blob/master/tensorflow_privacy/privacy/optimizers/dp_optimizer_keras_vectorized.py
# to support optimization of tf.float64 variables for numerical stability when jointly learning inducing variables and hyperparameters
  ...
  class DPOptimizerClass(cls):  # pylint: disable=empty-docstring
      ...
      with tf.keras.backend.name_scope(self._name + '/gradients'):
        ...
        def reduce_noise_normalize_batch(g):
          ...
          return tf.truediv(noised_gradient,
                            tf.cast(num_microbatches, tf.float64))
    ...
    def get_gradients(self, loss, params):
      ...
      def reduce_noise_normalize_batch(stacked_grads):
        ...
        noise_stddev = self._l2_norm_clip * self._noise_multiplier
        noise = tf.random.normal(
            tf.shape(input=summed_grads), stddev=noise_stddev, dtype=tf.float64)
        noised_grads = summed_grads + noise
        return noised_grads / tf.cast(num_microbatches, tf.float64)
